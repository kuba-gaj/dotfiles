...
Call `repowise_get_risk(targets=["path/to/file.py"])`. Per file it returns
...
- **Bug-fix history** (`defect_profile`) — present only on files with counted
  ...
  `bug_magnet` flag for sustained recent fix pressure, and `top_symbols`
...
- **Co-change partners** — files that change together with this one (often without
  an import link); you may need to update them too.
...
Batch all targets into one call: `repowise_get_risk(targets=["file1.py", "file2.py", "module/"])`.
...
Call `repowise_get_context(targets=["file.py"])` first to understand the full
context: what uses this file, what decisions govern it, and why it's structured
this way. This prevents accidentally violating architectural decisions. Also
check `repowise_get_why(targets=["file.py"])` before diverging from a pattern.

For a heavy refactor, also call `get_health` via the proxy
(`mcp({ server: "repowise", tool: "get_health", args: { targets: ["file.py"] } })`) —
the marker findings (complexity, deep nesting, low cohesion, duplication) tell
you *what* to improve while you're in there, and give you a before/after score.

## Error handling

If `get_risk` returns a tool error, the MCP server may not be running. Proceed
with the modification but note that risk assessment was unavailable.
